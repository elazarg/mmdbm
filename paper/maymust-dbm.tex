...
arise in closure rule candidates;
we treat such candidates as unusable because they do not denote a valid bound
in $\Rinf$; operationally, a closure update is applied only when the candidate
evaluates to a well-defined element of $\Rinf$. This is sound because closure
tightens entries only using semantically valid consequences of existing
constraints, and an undefined arithmetic form does not correspond to a valid
composed constraint.
...
\begin{remark}[Representation vs.\ Polarity of Information]
Although we classify variables by which \emph{direction} of information is
most important (E: upper bounds; A: guaranteed lower bounds), the internal
representation remains relational. In particular, $EE$ stores all pairwise
upper bounds on differences $e_i-e_j$, which also determines lower bounds via
negation (e.g., $\lower{(e_i)}=-EE[0,i]$). Likewise, $AA$ stores all pairwise
lower bounds on $a_i-a_j$, which determines upper bounds via negation.
The \emph{polarity-aware} aspect is the lattice structure (notably join/meet)
and the interpretation of which bounds are weakened at control-flow merges.
\end{remark}
...
\begin{lemma}[Coupling via the Shared Constant Zero]
\label{lem:two-zero-coupling}
Let $S=(EE,AA,M^+,M^-)$ be a state and let $\gamma(S)$ enforce
$\rho(e_0)=\rho(a_0)=0$ as in \Cref{def:gamma}. Then:
\begin{enumerate}
\item Any valuation $\rho$ satisfying the unary bounds in $EE$ and $AA$ induces
sound mixed bounds of the form in Rule~9.
\item Conversely, any valuation $\rho$ satisfying mixed bounds and the unary
bounds of the opposite class induces sound unary bounds of the form in
Rules~10--11.
\end{enumerate}
In this precise sense, Rules~9--11 are exactly the closure steps obtained by
allowing paths that pass through the shared constant $0$ (represented by both
$e_0$ and $a_0$).
\end{lemma}
...
\]
For closed states, we define the \emph{canonical join} as join-then-close:
\[
S_1 \join^* S_2 \;:=\; (S_1 \join S_2)^*.
\]
Closure is not an invariant of the lattice; $\join^*$ is used when a closed
representative is required (e.g., for the LUB statement below).
\end{definition}
...
\begin{remark}[Indexing Convention Used Below]
When we quantify $e_i$ we use $i \in [1,n_E]$; when we quantify $a_j$ we use
$j \in [1,n_A]$. Mixed matrix entries are then $M^\pm[i-1,j-1]$.
\end{remark}
...
For all $i,j \in [1,n_E]$ and $k \in [1,n_A]$:
\[
EE[i,j] \leftarrow \min\!\big(EE[i,j],\ M^+[i-1,k-1] + (-M^-[j-1,k-1])\big).
\]
This derives $\upper{(e_i - e_j)} \leq \upper{(e_i - a_{k})} + \upper{(a_{k} - e_j)}$.

\textbf{8. A via Mixed (A $\to$ E $\to$ A):}
For all $i,j \in [1,n_A]$ and $k \in [1,n_E]$:
\[
AA[i,j] \leftarrow \max\!\big(AA[i,j],\ (-M^+[k-1,i-1]) + M^-[k-1,j-1]\big).
\]
This derives $\lower{(a_i - a_j)} \geq \lower{(a_i - e_{k})} + \lower{(e_{k} - a_j)}$.
...
and $k \in [1, n_A]$ (intermediate $a_k$):
\begin{align*}
\rho(e_i) - \rho(e_j)
&= (\rho(e_i) - \rho(a_k)) + (\rho(a_k) - \rho(e_j)) \\
&\leq M^+[i-1, k-1] + (-M^-[j-1, k-1])
\end{align*}
using $\rho(e_i) - \rho(a_k) \leq M^+[i-1, k-1]$ and
$\rho(a_k) - \rho(e_j) \leq -M^-[j-1, k-1]$ (from
$\rho(e_j) - \rho(a_k) \geq M^-[j-1, k-1]$). Rule 8 is symmetric.
\end{proof}
...
One can compute closure in $O(n^3)$ time by a structured relaxation order
analogous to Floyd--Warshall, after viewing the system as a set of
difference constraints over a bipolar graph with reversal edges
(\Cref{rem:closure-graph}). We do not rely on this bound in the reference
implementation, which uses chaotic (worklist-style) iteration; the latter is
simple and monotone but may perform more than $O(n^3)$ relaxations in the
worst case. In practice, it terminates quickly for the intended verification
instances (see \Cref{sec:impl}).
\end{remark}
...
termination is guaranteed in finite-precision arithmetic because each entry is
updated monotonically (by $\min$ or $\max$) and thus can change only finitely
many times among representable floating-point values. See \Cref{sec:impl}.
\end{remark}
...
\begin{definition}[Component-wise Widening]
Let $S_1=(EE_1,AA_1,M^+_1,M^-_1)$ and $S_2=(EE_2,AA_2,M^+_2,M^-_2)$.
Define $S_1 \nabla S_2 = (EE,AA,M^+,M^-)$ entrywise by:
\begin{itemize}
\item For upper-bound components ($EE$, $M^+$):
\[
EE[i,j] =
\begin{cases}
EE_1[i,j] & \text{if } EE_2[i,j] \le EE_1[i,j] \\
+\infty & \text{otherwise}
\end{cases}
\qquad
M^+[p,q] =
\begin{cases}
M^+_1[p,q] & \text{if } M^+_2[p,q] \le M^+_1[p,q] \\
+\infty & \text{otherwise.}
\end{cases}
\]
\item For lower-bound components ($AA$, $M^-$):
\[
AA[i,j] =
\begin{cases}
AA_1[i,j] & \text{if } AA_2[i,j] \ge AA_1[i,j] \\
-\infty & \text{otherwise}
\end{cases}
\qquad
M^-[p,q] =
\begin{cases}
M^-_1[p,q] & \text{if } M^-_2[p,q] \ge M^-_1[p,q] \\
-\infty & \text{otherwise.}
\end{cases}
\]
\end{itemize}
...
\begin{remark}
This is the direct analogue of classical DBM widening: if an upper bound keeps
increasing, we drop it to $+\infty$; if a guaranteed lower bound keeps
decreasing, we drop it to $-\infty$. More precise widenings (e.g.,
threshold-based) can be plugged in component-wise.

However, care is needed with normalization after widening. Closure (and related
``strengthening'' steps in DBM-like domains) can re-tighten constraints that
widening just dropped, and naive iteration schemes that repeatedly apply
widening and full closure can behave poorly. A common practice is to apply
widening during fixpoint iteration while using closure sparingly (e.g., for
queries, at phase boundaries, or followed by a narrowing phase), or to use
threshold-based widenings.
\end{remark}
...
All unspecified off-diagonal entries are $-\infty$, expressing the absence of
a guaranteed lower bound on that difference.
...
\textbf{Safety check (integers):} $\upper{(\text{index})} = 5 < 7 = \lower{(\text{size})}$. Safe!
...
\subsection{Example 4: Mixed-to-Unary Propagation (Deriving an E Bound)}
...
The query $\texttt{check\_safe\_access}(S, i, j)$ is intended to establish that
$0 \le e_i < a_j$ holds for all $\rho \in \conc(S)$.

\textbf{Real semantics (sound sufficient check):}
\[
\texttt{check\_safe\_access}_{\R}(S,i,j) \;=\;
\big(\lower{(e_i)} \ge 0\big) \land \big(\upper{(e_i)} \le \lower{(a_j)}\big).
\]
This proves the non-strict variant $e_i \le a_j$; it is sufficient in contexts
where equality is safe or ruled out by a separate argument.

\textbf{Integer semantics (exact strict check):}
for integer-valued variables, $e_i < a_j$ is equivalent to $e_i-a_j \le -1$, so:
\[
\texttt{check\_safe\_access}_{\Z}(S,i,j) \;=\;
\big(\lower{(e_i)} \ge 0\big) \land \big(\upper{(e_i)} - \lower{(a_j)} \le -1\big).
\]
...
\midrule
Lower bounds & Represented implicitly via $0-x \le -L$ & Stored explicitly in $AA$ \\
Polarity & Implicit & Explicit (E vs A) \\
Cross-class constraints & All pairs uniformly & Interval on E--A \\
Join & $\max$ on stored upper-bound entries (lower bounds via negation) &
$\max$ on upper-bound components; $\min$ on lower-bound components \\
\bottomrule
\end{tabular}
